...
export default {
  async fetch(request, env) {
    ...
    if (url.pathname === "/version" && request.method === "GET") {
      return new Response(JSON.stringify({
        version: "V202.52.2",    // ★ V202.52.2 핫픽스 (Track A+B): Track A — (1) 시제 자동 교정 함수 fixTenseAlignment 추가 (워커), alignTenseWithLabel 줄단위 스캔 재설계 (클라이언트) — 라이브 결함 "PAST + 지금은~" / "PRESENT + 미래에는~" 자동 교정, (2) 인앱 배너 X 결함 3중 방어 — 결과 화면 표시 중 차단 + 결과 진입 시 잔존 배너 자동 정리 + X 버튼 이벤트 stopPropagation. Track B — (3) 매니페스토 #5 신설: 카테고리별 모범 톤 5개 추가 [썸/연락/짝사랑/재회/결혼/궁합] + 입력 이름 호명 규칙 + 무거운 카드(Tower/Devil/Death 등) 서브타입 톤 우선 가이드. V202.52.1 base 위에 적용.
        _ts: Date.now(),
        _ok: true
      }), {
        headers: {
