...
export default {
  async fetch(request, env) {
    ...
    if (url.pathname === "/version" && request.method === "GET") {
      return new Response(JSON.stringify({
        version: "V202.52.4",    // ★ V202.52.4 핫픽스 (V202.52.3 라이브 검증 추가 결함 4건): (#1) 자연어 의도 우선 분류 — '결혼/이별/재회' 키워드 + 두 사람 패턴 발견 시 compatibility 자동 추정 차단 (라이브: '결혼운' 입력 → 궁합 답 결함), (#2) 서브타입별 RELATION ESSENCE 라벨 매트릭스 — 이별/재회는 별도 톤 매핑 (라이브: 이별 95점에 '발전 흐름' 표시 결함), (#3) advance 톤 가이드 서브타입 어휘 분기 + 이별 '발전' 단어 금지 강화 (라이브: 이별 본문에 '조심스러운 발전' 결함), (#4) 호명 위치 강화 — ZEUS LOVE ORACLE 본문 1곳 1회만, 카드 본문 호명 절대 금지 (라이브: 카드 본문 + 결론 본문 2회 호명 결함). V202.52.3 base 위에 적용.
        _ts: Date.now(),
        _ok: true
      }), {
        headers: {
